...
# Instead of using the full belief transition matrix from system states to system states, use a matrix
# that goes fromsystem states to 3 states: 
# win (encapsulates states where the agent earns +1), 
...
class Agent:
    def __init__(self, idx, known_idx, N, ini_state, pos_states, Amx):
        ...
        sz = 2**len(known_idx)
        self.belief_mat = np.zeros((sz,3)) # Reduced matrix
        for i in range(sz):
            tot = 1
    ...
    def decide_action(self, state, e, C):
        ...
        if np.random.rand() >= e: #epsilon greedy
...
if __name__ == '__main__':
    ...
    #    print(agents[i].expected_payoff(state, '1'))
